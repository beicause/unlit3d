...
- `assert_image_snapshot` compares the frame against the snapshot named `name`
  and fails when the score is below `DEFAULT_MIN_SCORE` (85.0).
  `assert_image_snapshot_with_threshold` takes an explicit threshold.
...
compare against a snapshot that does not exist.
