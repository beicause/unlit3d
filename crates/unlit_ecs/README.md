...
hooks, no events or observers, no relations, and no scheduler. Everything that
would need one of those is done by the caller on the entities themselves.

The crate is at an **early stage of development** and its API changes freely.
It has no dependencies beyond a hasher and depends on nothing else in the
workspace, so it can be used on its own.
...
- **Two worlds, one implementation.** `LocalWorld` stores components in
  `RefCell`s and is `!Send`, so it stays on its thread — this is where a
  renderer and other thread-bound state belong. `SendWorld` stores them in
  `RwLock`s and is `Send + Sync`. The two differ only in their cells;
...
Reading and writing needs only `&World`; spawning and despawning need
`&mut World`, or a queued command:
...
`SendWorld` can be shared across threads. None of them needs a GPU, so they run
anywhere.
