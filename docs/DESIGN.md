...
综上，我们的上层API应该采取某种混合或折中模式，既要吸取OOP的优点，又要以符合Rust习惯的方式实现。

功能提议：
- 节点树API
  - 节点由组件组合而成，组件可以是数据，也可以是行为（闭包函数）。
  - 节点树自身也可以是节点，以组合多个节点树。
  - 节点树可以注册、解注册系统，以处理所有节点和驱动行为组件。可以用行为组件和系统实现类似godot的节点ready回调、帧更新、固定间隔更新、按键鼠标触摸输入等回调。
  - 并行性：一方面节点树的系统可能在不同线程执行，另一方面单个系统也可能以并行的方式执行行为组件。
  - 继承性：节点通过组合多个组件或组合多个节点实现数据和行为复用，即继承。
  - 节点树默认注册render系统。渲染器是行为组件，在render系统中渲染所有带有renderable的子节点，并输出到render target上（纹理或交换链）。相机是渲染器的一个数据组件。
