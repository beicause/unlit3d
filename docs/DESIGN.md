...
- 并行性，利用多线程。
- 易用性，能根据CPU场景数据同步至GPU缓冲并渲染。
- 允许底层控制，能绕过CPU数据直接更新GPU缓冲。
- 采用CPU视锥剔除。

反思其他引擎的做法：
- Bevy，ecs架构，模块化和拓展性好，能充分利用多线程。但是ecs架构有其缺点：
  1.组件变化检测机制复杂、易失效、易出错，组件依赖更新和派生机制几乎没有。如果每帧同步而不保留组件，开销可能较大。而如果保留组件，则涉及复杂的变化检测和依赖更新问题。例：`bevy_render`的提取范式易出错，尤其是在组件增删时，容易泄漏衍生组件，容易漏掉更新或使变化检测失效导致过度更新。复杂性体现在bevy的渲染系统往往很庞大，可能要处理大量组件，并带有大量`Changed`、`RemovedComponents`查询。
  2.bevy的渲染世界和主世界分离的设计，有助于并行，但也使得直接操作GPU资源变得困难，以及对于一次性初始化的GPU资源如buffer，texture，也要浪费CPU内存和提取开销。而且bevy的`RenderAssetUsages`机制复杂且易出错：主世界被提取了并移除了仅用于GPU的数据后，再访问它就会报错。
  3.拓展性体现在系统的相加上，而不在组件上。系统处理的组件类型无法拓展，缺乏OOP范式和动态性。例：bevy的材质拓展需要用户为每个自定义材质注册一个插件，较繁琐。bevy的材质所需的GPU句柄往往绑死了`ShaderBuffer`，`Mesh`，`Image`等类型的Handle，用户无法自定义自己的资源句柄用于材质。而这在OOP中，用基类并拓展非常容易。
  4.组件粒度的粗细难以把握。粒度过细则系统中查询参数多，粒度过粗则查询的组件带有多余信息。
- Godot、Three.js等：
