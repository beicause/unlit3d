...
- Godot、Three.js等，采用OOP、场景树/图和中心化渲染器，好的地方：易用性好，Godot的场景树轻松可以拆分和组合。API可以非常上层（如Godot的节点树），也可以比较底层（如Godot的服务器模式）。不好的地方：
  1.Rust本身不是OOP的，继承不方便，强行模拟OOP很别捏。
  1.不擅长处理大量实体，节点树开销较高。2.不像ECS那样自动和充分利用并行。并行是粗粒度的，需要手动调整。Godot的并行体现在服务器、节点的每帧更新逻辑可以选择后台线程，服务器和节点内部逻辑不能并行，除非手动调用线程池。

功能提议：
- 采用ECS
  - ECS框架保持精简，用Archetype储存。
  - 实体用Children，ChildOf组件引用其他实体表示父子关系。
  - 资源是组件，资源引用是实体引用。
  - 系统是包含行为（闭包函数）的组件。行为组件的调用时仅可访问自身实体上的组件和数据。
    多用每个实体上的行为组件，少用世界系统，实体像OOP的对象那样做好数据隔离。
  - 实体上的组件不能增删，只能读写。要改变实体上的组件种类必须despawn再重新spawn。
  - 没有事件/观察者，触发事件只是调用对应实体上的行为组件。
  - 世界系统是特殊的，可以访问所有实体上的组件，并且是普通行为组件的驱动者。例如可以用世界系统驱动行为组件，实现类似godot的节点内部的帧更新、固定间隔更新、按键鼠标触摸输入等回调。
  - render世界系统驱动渲染。渲染器是组件，在render系统中渲染所有带有renderable组件的孩子实体，并输出到render target上（纹理或交换链）。相机是渲染器的一个数据组件。
  - `!Send`世界和`Send`世界分离。只有`Send`世界的组件才是`Send`，`!Send`世界只能在主线程运行。GPU资源和渲染器是`!Send`组件，为了兼容`wasm32`。
