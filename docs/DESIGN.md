...
- Godot、Three.js等，采用OOP、场景图和中心化渲染器，好的地方：易用性好，Godot的场景图轻松可以拆分、组合和依赖。API可以非常上层（如Godot的节点图），也可以比较底层（如Godot的服务器模式）。不好的地方：
  1.不擅长处理大量实体，每个实例/节点内存不连续，开销较高。2.不像ECS那样自动和充分利用并行。并行是粗粒度的，需要手动调整。Godot的并行体现在服务器、节点的每帧更新逻辑可以选择后台线程，服务器和节点内部逻辑不能并行，除非手动调用线程池。
综上，我们的上层API建议采用类似Godot/Three.js的模式。

功能提议：
- 节点图API
  - 节点图自身也可以是节点，以组合多个节点图。
  - 节点需实现mount、unmount函数，在进入或退出节点图时触发。
  - 节点图可以注册、解除注册自定义回调。节点图公开调用接口以驱动节点图内节点的代码逻辑。
  - 用户可以注册、解除注册自定义节点图回调，可以实现类似godot的节点ready回调、帧更新、固定间隔更新、按键鼠标触摸输入等回调。这些回调均由外部调用节点图接口驱动，并且允许乱序并行而不依赖节点在图中的位置（本库不内置这些回调）。
  - 节点图默认注册render的回调。渲染器是节点，在render回调中渲染所有子节点，并输出到纹理或交换链上。相机本质是渲染器的相机矩阵ubo。
